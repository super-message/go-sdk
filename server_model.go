...
func QueryParameterFrom(r *http.Request) (q *QueryParameter, err error) {
	...
	// rt/rte/cid 是一定有的值
	q.RequestToken = strings.TrimSpace(query.Get("_rt"))
	if q.RequestToken == "" {
		return nil, errors.New("request token is required")
	}

	q.ChannelID = strings.TrimSpace(query.Get("_cid"))
	if q.ChannelID == "" {
	...
	q.TemplateID = strings.TrimSpace(query.Get("_tid"))

	templateVersion := query.Get("_tv")
	if templateVersion != "" {
